...
export class AdminHighlightForm implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      this.profileGateway.getHighlightById(Number(id)).subscribe((highlight) => {
  ...
  onSubmit(): void {
    ...
    const request$ = id
      ? this.profileGateway.updateHighlight(Number(id), values)
      : this.profileGateway.createHighlight(values);

    request$.subscribe(() => this.router.navigate(['/admin/about/highlights']));
  }
