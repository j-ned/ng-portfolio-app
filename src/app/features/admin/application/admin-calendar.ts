  ...
  template: `
    ...
    <div class="bg-surface border border-foreground/10 rounded-2xl p-6 shadow-lg mb-8">
      ...
      @if (bookings().length) {
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b border-foreground/10">
                <th
                  scope="col"
                  class="text-left py-3 px-3 text-xs font-semibold text-muted uppercase"
                >
                  Date
                </th>
                <th
                ...
                <th
                  scope="col"
                  class="text-right py-3 px-3 text-xs font-semibold text-muted uppercase"
                ></th>
              </tr>
            </thead>
            <tbody>
              @for (booking of bookings(); track booking.id) {
                <tr
                  class="border-b border-foreground/5 hover:bg-foreground/[0.02] transition-colors"
                >
                  <td class="py-3 px-3">
                    <span
                  ...
                  <td class="py-3 px-3">
                    <span
                      class="inline-block px-2 py-0.5 rounded bg-accent/10 text-accent text-xs font-medium"
                    >
                      {{ booking.startTime }} ({{ booking.duration }} min)
                    </span>
                  </td>
                  <td class="py-3 px-3 font-medium text-foreground">{{ booking.name }}</td>
                  <td class="py-3 px-3 text-muted">{{ booking.email }}</td>
                  <td class="py-3 px-3 text-muted max-w-[200px] truncate">
                    {{ booking.subject }}
                  </td>
        ...
        <p class="text-center text-muted py-8">Aucune réservation</p>
      }
    </div>
