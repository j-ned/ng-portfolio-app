...
10. **Ordre de deploiement API puis front** : le build front consomme l'API **prod** (`scripts/generate-sitemap.mjs`, `scripts/generate-rss.mjs`, `fetchPrerenderSlugs` dans `app.routes.server.ts`, prerender des pages) ; une PR front qui depend d'une evolution de l'API se merge **apres** que le nouveau conteneur API repond (tester l'endpoint en prod, ou `docker ps` sur `homeserver`), pas apres le simple merge de la PR API. Front merge trop tot = artefact statique fige sur l'ancienne reponse, a redeployer a la main depuis Dokploy (la CI GitHub ne le declenche pas, seul le push ou « Redeploy »). Verifier en prod apres deploiement ce qui est regenere au build (sitemap, RSS, HTML prerendu), pas seulement dans `dist/` (#118 merge a 16:03, API #39 deployee a 16:14 : enclosure RSS figee en `image/avif`).
